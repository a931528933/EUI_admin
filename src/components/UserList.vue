...
<el-dialog  title="编辑用户" :visible.sync="dialogFormVisible" >
  <el-form v-loading="loading" :model="form">
    ...
    <el-form-item label="区域" :label-width="formLabelWidth">
      <el-select v-model="form.area_id" placeholder="请选择用户类型">
    ...
    <el-form-item label="用户类型" :label-width="formLabelWidth">
      <el-select v-model="form.user_type" placeholder="请选择用户类型">
        ...
        <el-option label="社团团体" value="1"></el-option>
        <el-option label="院级机构" value="2"></el-option>
        <el-option label="校级机构" value="3"></el-option>
      </el-select>
    </el-form-item>
  </el-form>
...
export default {
  ...
  methods: {
    initTable(page) {
      ...
      query.user_name = this.query.queryStr;
      query.qq = this.query.queryStr;
      query.user_phone = this.query.queryStr;
      query.real_name = this.query.queryStr;
      ...
      this.loading = true;
      $.getJSON("/api/user/count", query, res => {
        this.totalPage = res.info;
      });
      $.getJSON("/api/user/all", query, res => {
        ...
        .then(() => {
          ...
          for (var i in remove) {
            var temp = new Promise(resolve => {
              $.getJSON("/api/user/remove", remove[i], res => {
                var result = res;
                resolve(result);
              });
            });
    ...
    handleDelete(id) {
      this.$confirm("删除, 是否继续?", "提示", {
        ...
        .then(() => {
          console.log(id);
          $.getJSON("/api/user/remove", { _id: id }, res => {
            if (res.success == 1) {
              this.$message({
                type: "success",
                message: "删除成功!"
    ...
    handleEdit(id) {
      ...
      this.loading = true;
      $.getJSON("/api/user/getOne", { _id: id }, res => {
